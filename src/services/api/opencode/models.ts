import { createOpencodeClient, type ModelInfo } from "./client"
...
export async function fetchFreeModels(client?: ReturnType<typeof createOpencodeClient>): Promise<ModelInfo[]> {
  const now = Date.now()
...
export function resolveModelId(config: OpenCodeClientConfig, models?: ModelInfo[]): string {
